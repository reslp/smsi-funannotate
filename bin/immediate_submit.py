...
if subs == "slurm":
	...
	slurm_args = "--partition={partition} --time={time} --qos={qos} --ntasks={ntasks} --cpus-per-task={cpus-per-task} --hint={hint} --output={output} --error={error} -n {n} -J {J} --mem={mem}".format(**job_properties["cluster"])
